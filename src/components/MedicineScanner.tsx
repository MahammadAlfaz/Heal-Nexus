...
import { scanMedicineImage } from '../utils/api';
...
export function MedicineScanner({ onNavigate, userType }: MedicineScannerProps) {
  ...
  const scanMedicine = async () => {
    ...
    try {
      const result = await scanMedicineImage(selectedImageFile);
      setScanResult(result);
    } catch (error) {
      console.error("Scan failed:", error);
      setScanResult({ isUnknown: true, error: (error as Error).message });
    } finally {
      setIsScanning(false);
    }
  };
  ...
  const speakText = (text: string, section?: string) => {
    ...
    try {
      ...
      console.log(`Attempting to find voice for language: ${selectedLanguage}, code: ${langCode}`);

      let voice = voices.find(v => v.lang === langCode);
      console.log(`Exact match for ${langCode}: ${voice ? voice.name : 'not found'}`);

      if (!voice) {
        voice = voices.find(v => v.lang.startsWith(langPrefix));
        console.log(`Prefix match for ${langPrefix}: ${voice ? voice.name : 'not found'}`);
      }

      if (!voice && langPrefix !== 'en') {
        voice = voices.find(v => v.lang.startsWith('en-'));
        console.log(`Fallback to English: ${voice ? voice.name : 'not found'}`);
      }
      ...
      if (voice) {
        ...
        console.warn(`No specific voice found for '${selectedLanguage}' in Medicine Scanner. Using browser default.`);
      } else {
                    ...
                    <div className="space-y-6">
                      <Alert className="border-orange-200 bg-orange-50">
                        ...
                        <AlertDescription className="text-orange-800">
                          <strong>{scanResult.error ? 'Scan Failed' : 'Unknown Medicine Detected'}</strong><br />
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['English', 'Hindi', 'Kannada', 'Tamil', 'Telugu', 'Bengali', 'Marathi', 'Gujarati', 'Malayalam'].map((lang) => (
                      <Button
                        key={lang}
                        size="sm"
